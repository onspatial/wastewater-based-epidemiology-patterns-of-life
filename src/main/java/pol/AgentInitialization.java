...
public class AgentInitialization {
	...
	public double generateDefecationRate() {
		UniformRealDistribution uRNG = new UniformRealDistribution(rng,
				params.defecationRateLowerBound, params.defecationRateUpperBound);
		return uRNG.sample();
	}
	...
	public double generateChanceOfInfection() {
		UniformRealDistribution uRNG = new UniformRealDistribution(rng,
				0.0, 1.0);
		return uRNG.sample();
	}

	public double generateChanceOfSpreading() {
		UniformRealDistribution uRNG = new UniformRealDistribution(rng,
				0.0, 1.0);
		return uRNG.sample();
	}

	public double generateSmoothnessRate() {
		UniformRealDistribution uRNG = new UniformRealDistribution(rng,
				0, 1.0);
		return uRNG.sample();
	}

	public double generateSheddingRate() {
		UniformRealDistribution uRNG = new UniformRealDistribution(rng,
				0, 1.0);
		return uRNG.sample();
	}
